...
def padding(images: np.array):
  ...
  max_shape = np.max(shapes, 0)  # Calculate the maximum shape across all images
...
def get_data_frame(vote_log: VoteLog, reader,):
  ...
  print(f'มีรายชื่อทั้งหมด {interested_row_b.sum()} รายชื่อ (ไม่นับหลังหมายเหตุ)')
...
def correct_party_df(vote_log: VoteLog, people_df):
  ...
  vote_log.data_table.loc[:, party_column] = vote_log.data_table[party_column].apply(fps)
...
def match_group_by_party(vote_log: VoteLog, people_df):
  ...
  for party, g in vote_log.data_table.groupby(party_column):
    clean_name_string_fn = lambda x: re.sub(title_re, '', re.sub(thai_char_re, '', x))
    ...
    for j, ocr_name in ocr_names.items():
      ...
      for i, row in filtered_people_df.iterrows():
        ...
        if name == ocr_name or name in ocr_name:
          ...
          break
        index_l.append(i);sim_l.append(similar(name, ocr_name))
